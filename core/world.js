...
(() => {
  ...
  chunkLogStats.interval = setInterval(() => {
    const loaded = chunkLogStats.loaded;
    const discarded = chunkLogStats.discarded;
  ...
  const World = {
    ...
    // Internal chunk logging stats
    _chunkLogStats: chunkLogStats,
    ...
    loadChunk(chunkX, chunkY) {
      ...
      // Aggregate log
      let entry = this._chunkLogStats.loaded.get(key);
      ...
      this._chunkLogStats.loaded.set(key, entry);
      return chunk;
    },
    ...
    cleanupChunks(keepChunkKeys) {
      for (const key of this.chunkCache.keys()) {
        if (!keepChunkKeys.has(key)) {
          let entry = this._chunkLogStats.discarded.get(key);
          if (!entry) entry = { count: 0, playerChunks: [] };
          entry.count++;
          entry.playerChunks.push(null);
          this._chunkLogStats.discarded.set(key, entry);
